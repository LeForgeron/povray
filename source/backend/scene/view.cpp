    ...
    */
    ...
    if(previewstartsize > 1)
    {
        ...
        for(unsigned int step = (previewstartsize >> 1); step >= previewendsize; step >>= 1)
        {
            ...
            for(int i = 0; i < maxRenderThreads; i++)
	        viewThreadData.push_back(dynamic_cast<ViewThreadData *>(renderTasks.AppendTask(new TraceTask(&viewData, 0, jitterscale, aathreshold, aadepth, aaGammaCurve, step, true, true, highReproducibility)))); // proposal for FS#313
        }
        ...
        if((previewendsize > 1) || (tracingmethod > 0))
        {
            ...
            for(int i = 0; i < maxRenderThreads; i++)
                viewThreadData.push_back(dynamic_cast<ViewThreadData *>(renderTasks.AppendTask(new TraceTask(&viewData, tracingmethod, jitterscale, aathreshold, aadepth, aaGammaCurve, 0, false, true, highReproducibility)))); // proposal for FS#313
        }
    }
