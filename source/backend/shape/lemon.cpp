 ...
 * lemon.cpp
 *
 * This module implements the lemon primitive.
 *
 * ---------------------------------------------------------------------------
...
bool Lemon::All_Intersections(const Ray& ray, IStack& Depth_Stack, TraceThreadData *Thread)
{
    ...
    if ((cnt = Intersect(P, D, I, Thread)) != 0)
    {
        for (i = 0; i < cnt; i++)
        {
            VEvaluateRay(Real_Pt, ray.Origin, I[i].d/len, ray.Direction);

            if (Clip.empty() || Point_In_Clip(Real_Pt, Clip, Thread))
            {
                Assign_Vector(INormal, I[i].n);
...
int Lemon::Intersect(const VECTOR & P, const VECTOR & D, LEMON_INT *Intersection, TraceThreadData *Thread) const
{
    int i = 0;
    DBL a, b, c[5], r[4];
    DBL d;
...
// intersection with apex and base disc
    if (Test_Flag(this, CLOSED_FLAG) && (fabs(D[Z]) > EPSILON))
    {
        ...
        {
            Intersection[i].d   = d ;
            Make_Vector(Intersection[i].n , 0, 0,-1);
            ++i;
        }
...
bool Lemon::Inside(const VECTOR IPoint, TraceThreadData *Thread) const
{
    DBL OCSquared;
...
void Lemon::Normal(VECTOR Result, Intersection *Inter, TraceThreadData *Thread) const
{
...
void Lemon::Translate(const VECTOR, const TRANSFORM *tr)
{
    Transform(tr);
}
...
void Lemon::Rotate(const VECTOR, const TRANSFORM *tr)
{
    Transform(tr);
}
...
void Lemon::Scale(const VECTOR, const TRANSFORM *tr)
{
    Transform(tr);
}
...
void Lemon::Compute_Lemon_Data(MessageFactory & messageFactory, pov_base::ITextStream *FileHandle, pov_base::ITextStream::FilePos & Token_File_Pos, int Token_Col_No )
{
    ...
    {
        VInverseScaleEq(axis, len);//normalize
    }
...
void Lemon::UVCoord(UV_VECT Result, const Intersection *Inter, TraceThreadData *Thread) const
{
...
void Lemon::CalcUV(const VECTOR IPoint, UV_VECT Result) const
{
    DBL len, x, y, z;
    DBL phi, theta;
    VECTOR P;
