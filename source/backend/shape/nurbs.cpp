...
RationalBezierPatch::RationalBezierPatch( const size_t x, const size_t y ): ObjectBase( RATIONAL_BEZIER_PATCH_OBJECT ), weight( x, y ), accuracy( 0.01 )
{
    ...
    wc[Y] = Grid( x, y );
    wc[Z] = Grid( x, y );
    Make_Vector( minbox , BOUND_HUGE, BOUND_HUGE, BOUND_HUGE );
    Make_Vector( maxbox , -BOUND_HUGE, -BOUND_HUGE, -BOUND_HUGE );
}
...
void RationalBezierPatch::evalNormal( Vector3d Real_Normal, const DBL u, const DBL v )const
{
    ...
    Vector3d v1,v2;
    Make_Vector (v1, cx2 / w2 - cx3 / w3, cy2 / w2 - cy3 / w3, cz2 / w2 - cz3 / w3 );
    vFirst = !vFirst;
    wc[X].point( u, v, vFirst, notused, cx2, cx3 );
    wc[Y].point( u, v, vFirst, notused, cy2, cy3 );
    wc[Z].point( u, v, vFirst, notused, cz2, cz3 );
    Make_Vector( v2, cx2 / w2 - cx3 / w3, cy2 / w2 - cy3 / w3, cz2 / w2 - cz3 / w3 );
    VCross( Normal_Vector , v1, v2 );
    VNormalizeEq(Normal_Vector);
...
void RationalBezierPatch::findPlanes( const BasicRay& ray, Vector3d& n1, DBL& d1, Vector3d& n2, DBL& d2 )const
{
    ...
    VNormalizeEq(n1);
    VCross( n2 , ray.Direction, n1 );
    VDot( d1, ray.Origin, n1 );
    d1 = -d1;
    VDot( d2, ray.Origin, n2 );
    d2 = -d2;
}
