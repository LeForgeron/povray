...
static DBL proximity_pattern (const VECTOR EPoint, const TPATTERN *TPat, TraceThreadData *Thread);
static DBL proportion_pattern (const VECTOR EPoint, const TPATTERN *TPat, TraceThreadData *Thread);
static DBL binary_pattern (const VECTOR EPoint, const TPATTERN *TPat, TraceThreadData *Thread);
static DBL voronoi_pattern (const VECTOR EPoint, const TPATTERN *TPat);
static DBL masonry_pattern (const VECTOR EPoint, const TPATTERN *TPat);
...
DBL Evaluate_TPat (const TPATTERN *TPat, const VECTOR EPoint, const Intersection *Isection, const Ray *ray, TraceThreadData *Thread)
{
	...
	switch(TPat->Type)
	{
		...
		case PROXIMITY_PATTERN:   value = proximity_pattern  (EPoint, TPat, Thread);                    break;
		case PROPORTION_PATTERN:  value = proportion_pattern (EPoint, TPat, Thread);                    break;
		case BINARY_PATTERN:      value = binary_pattern     (EPoint, TPat, Thread);                    break;
		case VORONOI_PATTERN:     value = voronoi_pattern    (EPoint, TPat);                            break;
		case MASONRY_PATTERN:     value = masonry_pattern    (EPoint, TPat);                            break;
...
void Copy_TPat_Fields (TPATTERN *New, const TPATTERN *Old)
{
	...
	*/
	...
	{
		if(Old->Vals.Proximity.Object != NULL)
		{
			New->Vals.Proximity.Object = reinterpret_cast<ObjectPtr>(Copy_Object(Old->Vals.Proximity.Object));
		}
	}
	
	if ((Old->Type == BINARY_PATTERN)||(Old->Type == PROPORTION_PATTERN))
	{
		New->Vals.Object_vect = new vector<ObjectPtr>;
		for(vector<ObjectPtr>::const_iterator c = Old->Vals.Object_vect->begin();
				c!=Old->Vals.Object_vect->end();c++)
		{
			New->Vals.Object_vect->push_back(reinterpret_cast<ObjectPtr>(Copy_Object(*c)));
		}
	}
  if ((Old->Type == VORONOI_PATTERN))
  {
     New->Vals.Point_vect = new vector<Vector3d>(*(Old->Vals.Point_vect));
  }
  if ((Old->Type == MASONRY_PATTERN))
  {
     New->Vals.Masonry.Cell_vect = new vector<Vector3d>(*(Old->Vals.Masonry.Cell_vect));
     New->Vals.Masonry.Cell_value = new vector<DBL>(*(Old->Vals.Masonry.Cell_value));
  }
...
void Destroy_TPat_Fields(TPATTERN *Tpat)
{
	...
	*/
	...
	{
		if(Tpat->Vals.Proximity.Object != NULL)
		{
			Destroy_Object(reinterpret_cast<ObjectPtr>(Tpat->Vals.Proximity.Object));
		}
	}
	...
	if ((Tpat->Type == BINARY_PATTERN)||(Tpat->Type == PROPORTION_PATTERN))
	{
		for(vector<ObjectPtr>::const_iterator c(Tpat->Vals.Object_vect->begin());
				c!=Tpat->Vals.Object_vect->end();c++)
		{
			Destroy_Object(*c);
		}
		Tpat->Vals.Object_vect->clear();
...
*
* DESCRIPTION
*   A bit lik object_pattern, for a collection of N objects.
...
static DBL proportion_pattern (const VECTOR EPoint, const TPATTERN *TPat, TraceThreadData *Thread)
{
	...
	unsigned int sum=0;
	for(vector<ObjectPtr>::const_iterator Current_obj=TPat->Vals.Object_vect->begin();
			Current_obj!=TPat->Vals.Object_vect->end();
			Current_obj++)
	{
		if(Inside_Object(EPoint, *Current_obj, Thread))
			sum ++;
		counter++;
	}
	if (counter)
...
static DBL voronoi_pattern (const VECTOR EPoint, const TPATTERN *TPat)
{
  ...
  unsigned int n=0;
	DBL square=0.0;
	DBL best_square;
  Vector3d focus(EPoint[X],EPoint[Y],EPoint[Z]);
	vector<Vector3d>::const_iterator Current=TPat->Vals.Point_vect->begin();
	best_square= (focus-(*Current)).lengthSqr();
  Current++;
      for(;
			Current!=TPat->Vals.Point_vect->end();
			Current++)
	{
    n++;
...
static DBL masonry_pattern (const VECTOR EPoint, const TPATTERN *TPat)
{
	...
	Vector3d focus(EPoint[X],EPoint[Y],EPoint[Z]);
	maxi = TPat->Vals.Masonry.Cell_vect->size();
  best_square=numeric_limits<DBL>::max();
  second_square=numeric_limits<DBL>::max();
  m= k = maxi;
	for(current=0;
			current<maxi;
			current++)
	{
		square= (focus-((*(TPat->Vals.Masonry.Cell_vect))[current])).lengthSqr();
		if (square < best_square)
		{
	...
	if ( ( (*(TPat->Vals.Masonry.Cell_value))[k] != (*(TPat->Vals.Masonry.Cell_value))[m] ) 
			&&(TPat->Vals.Masonry.Thickness > 0.0))
	{
		Vector3d dir(((*(TPat->Vals.Masonry.Cell_vect))[k])-((*(TPat->Vals.Masonry.Cell_vect))[m]));
		Vector3d mid((((*(TPat->Vals.Masonry.Cell_vect))[k])+((*(TPat->Vals.Masonry.Cell_vect))[m]))/2);
		...
		{ 
			if (vec.lengthSqr()>0.0)
			{
				dir.normalize();
				//vec.normalize();
				ratio = abs(dot(dir,vec)/((TPat->Vals.Masonry.Thickness)));
				if (ratio <= 1.0)
				{
					return (ratio*(TPat->Vals.Masonry.Middle))+((1.0-ratio)*(TPat->Vals.Masonry.Border));     
				}
			}
			else 
			{
				return (TPat->Vals.Masonry.Middle);     
			}
		}
	}

	return (*(TPat->Vals.Masonry.Cell_value))[k]; 
}
/*****************************************************************************
*
* FUNCTION
...
*
* DESCRIPTION
*   A bit lik object_pattern, with a cloud of points instead of just the
...
static DBL proximity_pattern (const VECTOR EPoint, const TPATTERN *TPat, TraceThreadData *Thread)
{
	if(TPat->Vals.Proximity.Object != NULL)
	{
		VECTOR TPoint; // test point
		TRANSFORM Trans;
		Compute_Rotation_Transform(&Trans,EPoint);
	  unsigned int sum=0;
		for(unsigned int i=0;i<PROXIMITY_CLOUD_SIZE;i++)
		{
			Make_Vector(TPoint, proximity_cloud[i].x, proximity_cloud[i].y, proximity_cloud[i].z);
			VScaleEq(TPoint,TPat->Vals.Proximity.Range/PROXIMITY_RADIUS);
			MTransPoint(TPoint,TPoint,&Trans);
			VAddEq(TPoint,EPoint);
			if(Inside_Object(TPoint, TPat->Vals.Proximity.Object, Thread))
